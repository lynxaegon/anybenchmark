...
function Metrics(requests) {
    ...
    this.count = 0                        // Number of sent messages
...
Metrics.prototype.error = function error(data) {
    ...
    let collection = this.errors[data.message];
...
Metrics.prototype.summary = function summary() {
    let results = new table.TableOutputStream({ columns: [
            { label: '', width: 20 },
            { label: '' }
        ]});
    ...
    //
    let width = (lrange[1] > hrange[1] ? lrange[1] : hrange[1]).toString().length;
    if (width < 6) width = 6;
    width++;
